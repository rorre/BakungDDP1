...
while True:
    ...
    if cmd == "INFO_TUJUAN":
    ...
    elif cmd == "TUJUAN_KELAS_TERMURAH":
        # Cek apabila argumen berisi 2, jika tidak maka
        # keluar dari command.
        if len(args) != 2:
            print("Perintah yang dimasukkan tidak valid.")
        ...
        # Cari harga termurah dan print semua jadwal termurah
        min_cost = min([sched[-1] for sched in filtered_schedule])
        cheapest_schedules = [
            sched for sched in filtered_schedule if sched[-1] == min_cost
        ]

        for sched in cheapest_schedules:
            print(schedule_format.format(sched[0], sched[2], sched[3]))

    elif cmd == "TUJUAN_JAM_TERMURAH":
        # Cek apabila argumen berisi 2, jika tidak maka
        # keluar dari command.
        if len(args) != 2:
            print("Perintah yang dimasukkan tidak valid.")
